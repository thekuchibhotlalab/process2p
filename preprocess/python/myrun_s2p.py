...
from suite2p import extraction, io, registration, detection, classification
from suite2p import version
...
def myrun_plane(ops, ops_path=None):
    ...
    if run_registration:
        ...
        if 'refImg' not in ops:
            ...
            print('Reference Image Detected --- Using Input Reference Image')
            refImg = ops['refImg']
            print(refImg.shape)
            ops = registration.register_binary(ops, refImg) # register binary with reference image
...
def myrun_s2p(ops={}, db={}, refImg = {}):
    ...
    if ops.get('multiplane_parallel'):
        ...
        for ipl, ops_path in enumerate(ops_paths):
            ...
            if type(refImg) is not dict:
                print(refImg.shape)
                op['refImg'] = refImg[:,:,ipl]
            print('>>>>>>>>>>>>>>>>>>>>> PLANE %d <<<<<<<<<<<<<<<<<<<<<<'%ipl)
            op = myrun_plane(op, ops_path=ops_path)
