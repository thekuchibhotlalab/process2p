function go_nogo_reinforced_probe_KKCD
...
%RP.RP.Halt; % Stops any processing chains running on RP2
%RP.RP.ClearCOF; % Clears all the buffers and circuits on that RP2
...
RP.write('numframesamples',numlicksamples);
RP.write('numlicksamples',numlicksamples);
RP.write('numwatersamples',numlicksamples);
RP.write('ToneOn',0);
...
switch moveforward
  ...
  case 'Go'
    ...
    %debugging
    %a=RP.read('FrameIndex');
    %b=RP.read('LickIndex');
    %c=RP.read('WaterIndex');
    
    %disp(['Frame Index (before start): ',num2str(a)]);
    %disp(['Lick Index (before start): ',num2str(b)]);
    %disp(['Water Index (before start): ',num2str(c)]);
...
%     zz = RP.write('ResetDataFrame',1);
%     zz = RP.write('ResetDataFrame',0);
%     
%     zz = RP.write('ResetDataLick',1);
%     zz = RP.write('ResetDataLick',0);
%     
%     zz = RP.write('ResetDataWater',1);
%     zz = RP.write('ResetDataWater',0);
    
    
    scanstart = zBus.zBusTrigB(0,0,5);
    disp(num2str(scanstart));
    ...
    %disp(['Frame Index (after start): ',num2str(a)]);
    %disp(['Lick Index (after start): ',num2str(b)]);
    %disp(['Water Index (after start): ',num2str(c)]);
...
%     
    ...
    for trialnum = 1:numtrials
        ...
        %disp(['Frame Index (trial' int2str(trialnum) ' start): ',num2str(a)]);
        %disp(['Lick Index (trial' int2str(trialnum) ' start): ',num2str(b)]);
        %disp(['Water Index (trial' int2str(trialnum) ' start): ',num2str(c)]);
...
%         disp(['Lickometer is now set to: ',num2str(RP.read('LickVal'))]);
        ...
        % Save data
        data = [trainingday trialnum tones(trialnum) response(trialnum) pretone_temp responsetime_temp delaytime_temp (toc - responsetime_temp) lickframe waterframe toc temp_toneon_framenumber context(trialnum)];
