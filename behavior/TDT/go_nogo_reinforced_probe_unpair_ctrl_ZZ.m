function go_nogo_reinforced_probe_unpair_ctrl_ZZ
...
solenoidOpen = 16; % number of ms to keep solenoid open
...
zBus=actxcontrol('ZBUS.x',[1 1 1 1]); %load zbus so we can trigger scanning later
zBus.ConnectZBUS('GB');
RP = TDTRP('C:\Users\kklab\Desktop\TPM_code_software_2018\TDT_NLW\MatlabSyncTone_LickDetect_Frame_KKCDZZ.rcx', 'RZ6', 'INTERFACE', 'UZ3');
...
switch moveforward
  ...
  case 'Go'
    ...
    % NEW CODE for start scanning - 11/21/2019
    scanstart = zBus.zBusTrigB(0,0,5);
    z = RP.write('DetectFrame',1); %trigger frame detection / writing
    z = RP.write('DetectLicks',1); %trigger lick detection / writing
    z = RP.write('DetectWater',1); %trigger water detection / writing
...
%                     RP.write('LickPosition',lickTubeIn);
        ...
        %-----------NEW REINF CONTEXT
        if context(trialnum) == 1
            %-----------NEW generate tone timing
            meanToneITI = 5.5;
            minToneITI = 2;
            toneITI = minToneITI + exprnd((meanToneITI-minToneITI),1,10);
            toneTiming = cumsum(toneITI);
            disp(['tone Time:' num2str(round(toneTiming,1))])
            toneFrames = round(toneTiming*framerate) + trialStartFrame;
            disp(['tone frame:' int2str(toneFrames)])
            toneID = randi(2,1,10);
            disp(['tone ID:' int2str(toneID)])
            tempToneFreq = [target, foil];
            tempToneAmp = [amps_target, amps_foil];
    ...
    %         disp(['Lickometer is now set to: ',num2str(RP.read('LickVal'))]);
            ...
            while RP.read('FrameNumber') < (currentframe+round(framerate)) 
                ...
                if any(toneFrames == RP.read('FrameNumber'))
                   % play tone
                    ...
                    a = RP.write('ToneOn',1);
                    this_temp_toneon_framenumber = RP.read('FrameNumber');
                    disp(['Tone starts at frame ',num2str(this_temp_toneon_framenumber)]);
                    tempToneTime = toc;

                    toneTime = [toneTime tempToneTime];
                    toneFramePlay = [toneFramePlay this_temp_toneon_framenumber];
                    toneIDPlay = [toneIDPlay toneID(toneIdx)];
            ...
            %---------NEW adda a delay to avoid tone-reward pairing
            prelick_delay = 0.5;
            pause(prelick_delay);
            %---------
    ...
    %         pause(toneduration + tonedelay);
            ...
            while RP.read('FrameNumber') < delayFrames
                % ---------NEW addded detection of tone timing
                if (tones(trialnum) == foil || lick == 0) && any(toneFrames == RP.read('FrameNumber'))
                    % play tone
                    tempFrame = RP.read('FrameNumber');
                    toneIdx = find(tempFrame == toneFrames);
                    disp(['The tone is ',num2str(toneFreqThisTrial(toneIdx)),' and attenuation is ',num2str(toneAmpThisTrial(toneIdx))]);
    ...
    %         disp(['Lickometer is now set to: ',num2str(RP.read('LickVal'))]);
            ...
            responsetime_temp = toc - pretone_temp;
            ...
            if tones(trialnum) == target
                if lick == 1 %hit
                    ...
                    responsetime(trialnum) = responsetime_temp;
                    lickframe = RP.read('FrameNumber');
                    while RP.read('FrameNumber') < (lickframe + 2), end %wait 2 frames before releasing water
                    while mod(RP.read('FrameNumber'),numoptotuneplanes) > 0, end  %wait in this while loop until currentframe is divisible by the number of optotune planes
                    d = RP.write('Solenoid',solenoidvalue); %Trigger and reset solenoid
                    d = RP.write('Solenoid',0);
                    waterframe = RP.read('FrameNumber');
            ...
            data = [trainingday trialnum tones(trialnum) response(trialnum) pretone_temp responsetime_temp delaytime_temp (toc - responsetime_temp) lickframe waterframe toc temp_toneon_framenumber context(trialnum) trialStartFrame trialEndFrame lickWindowFrame];
            dlmwrite(outfname,data,'-append');  
        end
    end
    %% ---------------------
    %RP.trg(1);
    scanend = zBus.zBusTrigB(0,0,5);
