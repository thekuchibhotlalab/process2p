...
allCovMatSpike = cell(1,length(sessionList));
...
for j = 1:length(baselineFile)
    ...
    for i = 1:nPlanes
        ...
        %tempDff = [tempDff;dff{i}(ishereBaseline{i},nFrames_oneplane(baselineFile(j),i)+1+(2-i):nFrames_oneplane(baselineFile(j)+1,i))];
...
tic;
nNeurons = size(spikeBaseline{j},1);
for j = 1:length(sessionList) % the first and last session
    ...
    allCovMatSpike{j} = cov(spikeBaseline{sessionList(j)}');
