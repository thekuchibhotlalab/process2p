function ToneEvokedActivityZZ1011(mouse)
...
for i=1:nDays_behav
    for k=1:2
        bef = squeeze(mean(S{i,k}(baseline,:,:))); % trials x neurons
        aft = squeeze(mean(S{i,k}(peakrew,:,:))); % trials x neurons
        ...
        mtracespos = squeeze(mean(S{i,k}(:,:,pttestpos<=0.05),2)); % psth x signif neurons 
        mtracesneg = squeeze(mean(S{i,k}(:,:,pttestneg<=0.05),2)); % psth x signif neurons 
